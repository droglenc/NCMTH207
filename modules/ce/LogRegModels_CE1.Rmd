...
cfs.bat <- cbind(Ests=coef(glm.bat),confint(boot.bat,type="perc"))
cfs.bat %>%
...
ggplot(data=bat,mapping=aes(x=canine,y=subsp01)) +
  ...
  scale_y_continuous(name="Probability of 'cinereus'",expand=expansion(mult=0.02)) +
  scale_x_continuous(name="Canine Tooth Height (mm)",expand=expansion(mult=0.02)) +
...
1. Interpret the exponentiated slope. Use the confidence interval.
...
1. Predict the log odds of being a *cinereus* if the canine tooth height is 4 but only using your previous prediction and the value of the slope.
1. Predict the odds of being a *cinereus* if the canine tooth height is 4 but only using your previous predication and the value of the exponentiated slope.
