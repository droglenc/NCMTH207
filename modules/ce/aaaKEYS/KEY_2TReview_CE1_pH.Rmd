...
1. The p-value is `r kPvalue(rvr.t$p.value,include.p=FALSE,latex=FALSE)` (or more specifically `r formatC(rvr.t$p.value)`). This p-value is less than &alpha;=0.05, which means that H<sub>0</sub> is rejected and it appears that the mean pH is greater for stream A than for stream B.
