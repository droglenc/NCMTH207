...
<ul>
  <li>On the first question, define any symbols used in the hypotheses.</li>
  ...
  <li>In the fifth question include a plot of Tukey's corrected means with confidence intervals.</li>
  ...
  <li>Also note in the fourth, fifth, and last questions that the conclusions are about the MEAN days to hatch not days to hatch because that is what is tested in the hypotheses. Make sure to include the word MEAN.</li>
...
1. It appears that the mean days to hatch differs between 30<sup>o</sup>C and 15<sup>o</sup>C (`r kPvalue(mc1$contrasts$p.value[3],latex=FALSE)`), 20<sup>o</sup>C (`r kPvalue(mc1$contrasts$p.value[5],latex=FALSE)`), and 25<sup>o</sup>C (`r kPvalue(mc1$contrasts$p.value[6],latex=FALSE)`). The mean days to hatch did not differ between any other pairs of temperature treatments (p>`r kPvalue(min(mc1$contrasts$p.value[c(1,2,4)]),latex=FALSE,include.p=FALSE)`).
...
1. It appears that the mean total length differs only between the control (no clipping) treatment and the "large" (10.0-mm) clipping treatment (`r kPvalue(mcS2$contrasts$p.value[3],latex=FALSE)`). The mean total length did not differ between any other pairs of tail clipping treatments (p>`r kPvalue(min(mcS2$contrasts$p.value[-3]),latex=FALSE,include.p=FALSE)`).
