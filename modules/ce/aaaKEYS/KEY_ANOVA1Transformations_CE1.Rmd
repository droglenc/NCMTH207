...
modHTML_CEKeys("KEY_ANOVA1Transformations_CE1")
```
...
<ul>
  ...
  <li>In question 2 be careful to state that it is the mean of the transformed variable that differs (not the mean of the untransformed variable).</li>
  <li>Use Tukey's multiple comparison results when identifying specifically which pairs of group means differ.</li>
  <li>In questions 3-5 you must specifically refer to the mean of the transformed variable.</li>
  <li>Note that when the square root transformation was used that the group mean could be back-transformed (question 6) but the difference in group means could not (question 7).</li>
  <li>Note that when the log transformation was used that both the group mean (question 6) and the difference in group means (question 7) could be back-transformed.</li>
  <li>I find it hard to interpret the back-transformed difference in means (from the log scale) when the ratio is less than 1. Thus, I prefer to flip the ratio and talk about how the second group is that many times larger than the first group.</li>
</ul>
</div>

# Assumptions I

```{r Assumptions1, echo=FALSE, warning=FALSE, message=FALSE}
...
mc1t <- emmeans(lm1t,specs=pairwise~group,tran="sqrt")
...
1. The mean square root of the measurement variable for the "A" group is between `r formatC(mcsum1t$emmeans$lower.CL[1],format="f",digits=2)` and `r formatC(mcsum1t$emmeans$upper.CL[1],format="f",digits=2)`.
1. The mean square root of the measurement variable is betwen `r formatC(-1*mcsum1t$contrasts$upper.CL[1],format="f",digits=3)` and `r formatC(-1*mcsum1t$contrasts$lower.CL[1],format="f",digits=3)` units less for the "A" group than for the "B" group.
1. The back-transformed mean of the measurement variable for the "A" group is between `r formatC(mcsum1bt$emmeans$lower.CL[1],format="f",digits=2)` (=`r formatC(mcsum1t$emmeans$lower.CL[1],format="f",digits=2)`<sup>2</sup>) and  `r formatC(mcsum1bt$emmeans$upper.CL[1],format="f",digits=2)` (=`r formatC(mcsum1t$emmeans$upper.CL[1],format="f",digits=2)`<sup>2</sup>).
...
mc2t <- emmeans(lm2t,specs=pairwise~group,tran="log")
mcsum2t <- summary(mc2t,infer=TRUE)
mcsum2bt <- summary(mc2t,infer=TRUE,type="response")
...
1. The mean log of the measurement variable for the "A" group is between `r formatC(mcsum2t$emmeans$lower.CL[1],format="f",digits=2)` and  `r formatC(mcsum2t$emmeans$upper.CL[1],format="f",digits=2)`.
1. The mean log of the measurement variable is between `r formatC(mcsum2t$contrasts$lower.CL[1],format="f",digits=3)` and  `r formatC(mcsum2t$contrasts$upper.CL[1],format="f",digits=3)` units greater for the "A" group than for the "B" group.
1. The back-transformed mean of the measurement variable for the "A" group is between `r formatC(mcsum2bt$emmeans$lower.CL[1],format="f",digits=2)` (=e<sup>`r formatC(mcsum2t$emmeans$lower.CL[1],format="f",digits=2)`</sup>) and  `r formatC(mcsum2bt$emmeans$upper.CL[1],format="f",digits=2)` (=e<sup>`r formatC(mcsum2t$emmeans$upper.CL[1],format="f",digits=2)`</sup>).
1. The back-transformed mean of the measurement variable for the "A" group is between `r formatC(mcsum2bt$contrasts$lower.CL[1],format="f",digits=3)` and  `r formatC(mcsum2bt$contrasts$upper.CL[1],format="f",digits=3)` TIMES larger than the back-transformed mean for the "B" group.
...
mc3t <- emmeans(lm3t,specs=pairwise~use,tran="log")
mcsum3t <- summary(mc3t,infer=TRUE)
mcsum3bt <- summary(mc3t,infer=TRUE,type="response")
...
1. The mean log concentration of iron was lowest in streams in the unmined basin at between `r formatC(mcsum3t$emmeans$lower.CL[1],format="f",digits=3)` and `r formatC(mcsum3t$emmeans$upper.CL[1],format="f",digits=3)`.
1. The back-transformed mean concentration of iron in streams in the unmined basin is between `r formatC(mcsum3bt$emmeans$lower.CL[1],format="f",digits=3)` mg/L (=e<sup>`r formatC(mcsum3t$emmeans$lower.CL[1],format="f",digits=3)`</sup>) and `r formatC(mcsum3bt$emmeans$upper.CL[1],format="f",digits=3)` mg/L (=e<sup>`r formatC(mcsum3t$emmeans$upper.CL[1],format="f",digits=3)`</sup>).
1. The mean log concentration of iron in streams in the unmined basin was between `r formatC(-1*mcsum3t$contrasts$upper.CL[2],format="f",digits=3)` and  `r formatC(-1*mcsum3t$contrasts$lower.CL[2],format="f",digits=3)` units less than the mean log concentration of iron in streams in basins with abandoned mines.
1. The mean concentration of iron in streams in the unmined basin was between `r formatC(mcsum3bt$contrasts$lower.CL[2],format="f",digits=3)` and `r formatC(mcsum3bt$contrasts$upper.CL[2],format="f",digits=3)` as large as the mean concentration of iron in streams in the basin with abandoned mines. Alternatively, mean concentration of iron in streams in the basin with abandoned mines was between `r formatC(1/mcsum3bt$contrasts$upper.CL[2],format="f",digits=3)` and `r formatC(1/mcsum3bt$contrasts$lower.CL[2],format="f",digits=3)` times larger than the mean concentration of iron in streams in the unmined basin.
