...
Measuring the live body weight of large-bodied animals can be impractical when equipment needed to weigh individuals is inadequate or unavailable. Thus, the objective of this analysis is to determine if body weight can be predicted from body length for non-cub bears (i.e., older than one year of age). 
...
The log weight of a bear increases between `r formatC(cfs.bbt["loglength","2.5 %"],format="f",digits=3)` and `r formatC(cfs.bbt["loglength","97.5 %"],format="f",digits=3)` for each unit increase in log body lengt of the bears. More importantly the weight is multiplied by a value between `r formatC(btcfs.bbt["loglength","2.5 %"],format="f",digits=3)` and `r formatC(btcfs.bbt["loglength","97.5 %"],format="f",digits=3)` for each 2.718 unit increase in length of the bear.
