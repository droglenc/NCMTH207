...
<strong>Note:</strong> 
<ul>
  <li>Make sure you use the "ln" and not the "log" button on your calculator. We always use natural logs in this class.</li>
</ul>
</div>
...
cfs.bat <- round(cbind(Ests=coef(glm.bat),confint(glm.bat)),3)
...
odds4 <- odds3*ebeta
x50 <- -alpha/beta
x95 <- predX(0.95,alpha,beta)
x5 <- predX(0.05,alpha,beta)

```

1. The equation of the best-fit logistic regression line if $log\left(\frac{p}{1-p}\right)$=`r alpha`+`r beta`&times;canine, where p is the probability that the bat is of the *cinereus* subspecies and canine is the height of the canine tooth in mm.
...
1. The canine tooth height where it is “50-50” that the bat is *cinereus* is **`r formatC(x50,format="f",digits=2)` mm** (=$\frac{-(`r alpha`)}{`r beta`}$).
1. The canine tooth height where you are “very sure” (95% likely) that the bat is a *cinereus* is **`r formatC(x95,format="f",digits=2)`** mm (=$\frac{log\left(\frac{0.95}{0.05}\right)-(`r alpha`)}{`r beta`}$).
1. The canine tooth height where you are “very sure” (95% likely) that the bat is a *semotus* (i.e., not a *cinereus*) is **`r formatC(x5,format="f",digits=2)` mm** (=$\frac{log\left(\frac{0.05}{0.95}\right)-(`r alpha`)}{`r beta`}$).
