...
<ul>
  ...
  <li>When discussing models, means, or residuals make sure to explicitly state what type of mean you are talking about. Say "group means" or "grand mean" not just "mean."</li>
</ul>
</div>
...
1. The residual MS for the grand mean model is $\frac{`r dss$ss_total`}{`r dss$df_total`}$=**`r formatC(dss$ms_total,format="f",digits=2)`**. The square root of this value is approximately equal to the mean of the absolute values of the residuals (from the grand mean).
...
1. The residual MS for the group means model is $\frac{`r dss$ss_within`}{`r dss$df_within`}$=**`r formatC(dss$ms_within,format="f",digits=2)`**. The square root of this value is approximately equal to the mean of the absolute values of the residuals (from the observation's group mean).
...
1. The RSS<sub>Total</sub>-RSS<sub>Within</sub> is `r dss$ss_total`-`r dss$ss_within`=`r dss$ss_total-dss$ss_within`. This value is the reduction in RSS from using the group means rather than the grand means. It is the improvement in "lack-of-fit" from using the groupp means rather than the grand mean or how much better the group means model fits compared to the grand mean model.
1. The df<sub>Total</sub>-df<sub>Within</sub> is `r dss$df_total`-`r dss$df_within`=`r dss$df_total-dss$df_within`. This value is the difference in number of parameters between the group means and grand mean models. It is how much more complex the group means model is than the grand mean model.
