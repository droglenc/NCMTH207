...
The \R{powerSim()} function provides sliders that can be used to determine the effect of changing the sample size ($n$), Type I error rate ($\alpha$), and natural variability ($\sigma$) on power.  Use the default settings of this function to explore the following questions.
...
p3 <- sapply(deltas,power.t.test,n=NULL,power=0.8,sig.level=0.05,sd=25,type="two.sample",
             alt="one.sided")
p3.n <- as.numeric(p3["n",])
plot(p3.n~perc.delta,xlab="Percentage Population Decline (from 100)",ylab="Sample Size",
     type="l",ylim=c(0,max(p3.n)))
abline(h=0,lty=2)
