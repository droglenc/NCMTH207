...
<ul>
  ...
  <ul>
    <li>There is a significant relationship between the log(odds) of accepting the offer and family income (`r kPvalue(sum[2,"Pr(>|z|)"])`;)</li>
  </ul>
...
x25ci <- quantile(x25,c(0.025,0.975))
```

  <li>At what family income will 25% of the residents accept the offer?</li>
  <ul>
    <li>The offer will be accepted by 25% of the residents when their family income is `r formatC(x25,format="f",digits=0)`, with a 95% confidence interval from `r formatC(x25ci[1],format="f",digits=0)` to `r formatC(x25ci[2],format="f",digits=0)`.</li>
  </ul>
