...
# Model Purposes
As seen above, each model uses different types of data. Not surprisingly then, each model is used to test different hypotheses or has a different analytical purpose. These purposes will be discussed in detail in subsequent modules. However, the major objective of each module is explained briefly below (in the order that we will cover the models).

Each example uses a data set that contains data about [mirex](https://en.wikipedia.org/wiki/Mirex) concentrations (`mirex`) for two species of salmon (`species`) captured in six years between 1977 and 1999 (`year`) in Lake Ontario. In addition the weight of each fish (`weight`) and whether or not the mirex concentration exceeded the EPA limit of 0.1 mg/kg (`exceeds_limit`) were recorded.

&nbsp;

A **one-way ANOVA** is used to determine if the mean of the response variable (`mirex`) differs among two or more groups defined by a single categorical variable (e.g., `year`).
...
A **two-way ANOVA** is used to determine if the mean of the response variable (`mirex`) differs among groups for one categorical variable (e.g., `year`), another categorical variable (e.g., `species`), or the interaction between the two categorical variables.
...
A **simple linear regression** is used to determine if there is a relationship between the response variable (e.g., `mirex`) and the single quantitative explanatory variable (e.g., `weight`).
...
An **indicator variable regression** is used to determine if the simple linear regressions differ between two or more groups (`species`).
...
A **logistic regression** is used to determine if the probability of "success" for the binary categorical response (e.g., `exceeds_limit`) is related to the quantitative explanatory variable (e.g., `weight`).
