...
1. Construct an ANOVA table for the full model with `anova()` and make a conclusion about the equality of means from the p-value.
1. If differences among group means exist, then use a multiple comparison technique with `emmeans()` and `summary()` to identify specific differences. Discuss specific differences **using confidence intervals**.
...
1. Create a summary graphic of observations with group means **on the original scale** and 95% confidence intervals using `ggplot()` and results from `emmeans()`.
...
Variances among treatments for the untransformed data appear to be non-constant (Levene's `r kPvalue(p.lev.af,latex=FALSE)`; Figure \@ref(fig:AFAssumptions1)-Right). The residuals appear to be not normally distributed (Anderson-Darling `r kPvalue(p.ad.af,latex=FALSE)`) with a fairly strong skew in the histogram (Figure \@ref(fig:AFAssumptions1)-Left). There also appears to be a significant outlier (outlier test `r kPvalue(p.out.af,latex=FALSE)`) that has a large residual and appears to be in the Oak group (Figure \@ref(fig:AFAssumptions1)). None of the assumptions are met so transforming the food rate was considered.
...
Tukey multiple comparisons indicate that the mean log foraging rate was greater for ants on Oak trees than ants on Rowan trees (`r kPvalue(dmns.aft$p.value[1],latex=FALSE)`; Table \@ref(tab:AFTukey1)). Specifically, the mean foraging rate for ants on an Oak tree was between `r formatC(dmns.afbt$lower.CL[1],format="f",digits=3)` and `r formatC(dmns.afbt$upper.CL[1],format="f",digits=3)` **times** higher than the mean foraging rate for ants on a Rowan tree (Table \@ref(tab:AFTukey2)). However, there was no significant difference in mean log foraging rate between ants on Sycamore trees and ants on Oak (`r kPvalue(dmns.aft$p.value[2],latex=FALSE)`) or Rowan (`r kPvalue(dmns.aft$p.value[3],latex=FALSE)`; Table \@ref(tab:AFTukey1)) trees. 
