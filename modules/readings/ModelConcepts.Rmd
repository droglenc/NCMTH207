...
The RSS is on an unfamiliar scale (squared residuals?) but it maintains the same conceptual idea that summing residuals would have. Mainly, the smaller the RSS the more closely the points are to the model. The full set of residuals required to compute an RSS are shown in \@ref(fig:RSS1).
...
In the example above, there were four observations (n) and one model parameter -- x&#772; -- so df=4-1=3. In Figure \@ref(fig:TwoModelsEx)-Left there are `r nrow(aqua)` observations and two parameters (i.e., two group means) so Rdf=`r nrow(aqua)`-2=`r nrow(aqua)-2`. In Figure \@ref(fig:TwoModelsEx)-Right there are `r nrow(Mirex)` observations and two parameters (i.e., the slope and intercept of the regression line) so Rdf=`r nrow(Mirex)`-2=`r nrow(Mirex)-2`.
