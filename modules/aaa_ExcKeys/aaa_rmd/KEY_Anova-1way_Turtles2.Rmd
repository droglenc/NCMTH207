...
1. The mean days to hatch is significantly different among the four temperature groups (`r kPvalue(aov1[1,"Pr(>F)"],digits=3,latex=FALSE)`; `r tbls("aov",display="cite")`).
1. It appears that the mean days to hatch for the 30<sup>o</sup>C group is significantly lower than the mean days to hatch for 15<sup>o</sup>C (`r kPvalue(mc1p[3,"p value"],digits=3)`), 20<sup>o</sup>C (`r kPvalue(mc1p[5,"p value"],digits=3)`), and 25<sup>o</sup>C (`r kPvalue(mc1p[6,"p value"],digits=3)`) groups (`r tbls("mcp",display="cite")`). The mean days to hatch for all other pairs of temperature groups are not significantly different (`r tbls("mcp",display="cite")`). These results are shown visually in `r figs("fitplot",display="cite")`.
