...
1. The Levene's test suggests that the variances are equal among the temperature groups (`r kPvalue(lev1[1,"Pr(>F)"],digits=3,latex=FALSE)`). The boxplots of residuals are very roughly equal in height and thus support the conclusion of equal variances (`r figs("residplot",display="cite")`-Right).
1. The Anderson-Darling normality test suggests that the residuals are normally distributed (`r kPvalue(ad1$p.value,digits=3,latex=FALSE)`). The histogram of the residuals is slightly (but not strongly) left-skewed (`r figs("residplot",display="cite")`-Left). The Anderson-Darling tests though is definitive and, thus, the residuals appear to be normal.
1. There are no significant outliers (`r kPvalue(out1$bonf.p,digits=3,latex=FALSE)`), though the residual plot suggests possible outliers in the 25<sup>o</sup>C group (`r figs("residplot",display="cite")`).
