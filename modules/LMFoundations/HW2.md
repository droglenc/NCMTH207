...
1. How do the p-values for the *slope* (in the coefficients table), the ANOVA table, and the 2-Sample t-Test compare?  Explain why this relationship occurs. [*Hint: You will need to discuss the H<sub>0</sub> and H<sub>A</sub> for each p-value and explain how they are equivalent.*]
1. What is the overall conclusion about the group means from these p-values?
...
1. How does the df from the 2-Sample t-Test compare to one of the df in the ANOVA table (again,  but be be specific about which one).  Explain why this relationship occurs. [*Hint: You will need to discuss how these df are computed.*]
