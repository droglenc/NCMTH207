...
The remainder of this module is devoted to reviewing basic concepts from your introductory statistics course and developing a common general framework that can be used to analyze all four types of linear models described above. Thus, this module serves as the theoretical foundation for Modules \ref{chap:LMANOVA1}-\ref{chap:LMLogistic}.
