...
\begin{Itemize}
  ...
  \item $\beta_{1}$: coefficient on the covariate.
...
lines(c(0,1,1),c(a,a,a+b1),lty=3,lwd=3)
text(1,a+b1/2,expression(beta[1]),pos=4,cex=1.3)     # 1st slope (beta)
lines(c(0,1,1),c(a+d1,a+d1,a+d1+b1+g1),
      lty=3,lwd=3,col="blue")                        # 2nd slope (beta + gamma)
text(1,a+d1+(b1+g1)/2,expression(beta[1]+gamma[1]),
...
lines(c(x,x+1,x+1),c(a,a,a+b1),lty=3,lwd=3)
text(x+1,a+b1/2,expression(beta[1]),pos=4,cex=1.3)  # beta of beta+gamma slope
lines(c(x+1,x+1),c(a+b1,a+b1+g1),
...
The model expressed in \eqnref{eqn:IVRSalmonModel1} is then fit and the coefficient estimates and confidence intervals are extracted with \R{coef()} and \R{confint()}.\footnote{The order of the variables should be as discussed previously -- covariate followed by the indicator variable.} Thus, $\hat{\alpha}$=\Sexpr{formatC(cf.m1[1],format="f",digits=4)}, $\hat{\beta}_{1}$=\Sexpr{formatC(cf.m1[2],format="f",digits=4)}, $\hat{\delta}_{1}$=\Sexpr{formatC(cf.m1[3],format="f",digits=4)}, and $\hat{\gamma}_{1}$=\Sexpr{formatC(cf.m1[4],format="f",digits=4)}. Thus, for example, the sample slope for chinook salmon is \Sexpr{formatC(cf.m1[2],format="f",digits=4)} and the sample slope for coho salmon is \Sexpr{formatC(-1*cf.m1[4],format="f",digits=4)} smaller than the sample slope for chinook salmon. Furthermore, one is 95\% confident that $\beta_{1}$ is between \Sexpr{formatC(ci.m1[2,1],format="f",digits=4)} and \Sexpr{formatC(ci.m1[2,2],format="f",digits=4)} and $\gamma_{1}$ is between \Sexpr{formatC(ci.m1[4,1],format="f",digits=4)} and \Sexpr{formatC(ci.m1[4,2],format="f",digits=4)}.
<<>>=
mirex.lm1 <- lm(mirex~weight*species,data=Mirex)
...
@
The initial ultimate full model fit to these data was
\[ \mu_{totlen|footlen,pop} = \alpha + \beta_{1}footlen + \delta_{1}popother + \gamma_{1}popother*footlen  \]
...
The reduced model
\[ \mu_{totlen|footlen,pop} = \alpha + \beta_{1}footlen + \delta_{1}popother  \]
