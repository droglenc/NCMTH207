%===============================================================================
% swv2PDF("04_02_HW_LMRegression1.rnw",createScript=FALSE)
%===============================================================================
...
\section*{Question 4.2 (6 pts)}
<<echo=FALSE,results=hide>>=
year <- 1970:1990
propmale <- c(0.5134,0.5126,0.5125,0.5128,0.5133,0.5132,0.5128,0.5128,0.5129,0.5127,0.5129,0.5126,0.5123,0.5127,0.5122,0.5126,0.5122,0.5120,0.5121,0.5120,0.5120)
...
\end{table}
      ...
      \item The proportion of males \textbf{declined} between \Sexpr{formatC(-1*ci1["year",2],format="f",digits=6)} and \Sexpr{formatC(-1*ci1["year",1],format="f",digits=6)} per year \tabrefp{tab:LMSLRci1}.
\begin{table}[!h]
  \centering
...
\section*{Question 4.3 (8 pts)}
<<echo=FALSE,results=hide>>=
mass <- c(3.33,4.62,5.43,5.73,6.12,6.29,6.45,6.51,6.65,6.75,6.81,7.56,7.83,8.02,8.06,8.18,9.08,9.15,9.35,9.42,9.95)
t.cell <- c(0.252,0.263,0.251,0.251,0.183,0.213,0.332,0.203,0.252,0.342,0.471,0.431,0.312,0.304,0.370,0.381,0.430,0.430,0.213,0.508,0.411)
...
<<eval=FALSE>>=
year <- 1970:1990
propmale <- c(0.5134,0.5126,0.5125,0.5128,0.5133,0.5132,0.5128,0.5128,0.5129,0.5127,
0.5129,0.5126,0.5123,0.5127,0.5122,0.5126,0.5122,0.5120,0.5121,0.5120,0.5120)
d <- data.frame(year,propmale)
lm1 <- lm(propmale~year,data=d)
summary(lm1)
confint(lm1)
@
<<eval=FALSE>>=
mass <- c(3.33,4.62,5.43,5.73,6.12,6.29,6.45,6.51,6.65,6.75,6.81,7.56,7.83,8.02,
8.06,8.18,9.08,9.15,9.35,9.42,9.95)
t.cell <- c(0.252,0.263,0.251,0.251,0.183,0.213,0.332,0.203,0.252,0.342,0.471,
0.431,0.312,0.304,0.370,0.381,0.430,0.430,0.213,0.508,0.411)
d <- data.frame(mass,t.cell)
lm2 <- lm(t.cell~mass,data=d)
summary(lm2)
confint(lm2)
predict(lm2,data.frame(mass=5),interval="c")
predict(lm2,data.frame(mass=5),interval="p")
@
