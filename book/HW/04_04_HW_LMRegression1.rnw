...
@
  \begin{Enumerate}
    ...
    \item The Anderson-Darling test shows weak but insignificant evidence for non-normality (\Sexpr{kPvalue(ad1$p)}) and the histogram of residuals is approximately symmetric (\figref{fig:carresidplot2}-Right).
