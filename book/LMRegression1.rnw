...
\turnpage{36}
  \item \label{hwprob:LMSLRBirths} \cite{Davisetal1998} examined the proportion of male births (PropMale) in several industrialized nations.  The data for the United States are shown below.
...
\vspace{30pt}
  \item \label{hwprob:LMSLRCarMPG2} Continue with the car data collected by students in the Introduction to Environmental Studies course begun in \ref{hwprob:LMSLRCarMPG1}.
    \begin{Enumerate}
